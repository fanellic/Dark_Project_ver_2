        ...
        if (JennyController.isGrounded)
        {
            if ((dotProductLeft < -0.1f || dotProductRight < -0.1f) && JennyController.movementVectorRef != new Vector2(0f, 0f))
            {
                ...
                if (leftSide > rightSide)
                {
                    rightXPush = -.25f;
                    leftXPush = 1.1f;
                    rightUpPush = (transform.root.InverseTransformPoint(leftShinBone.transform.position).y) / stupid ;
                    leftUpPush = (transform.root.InverseTransformPoint(leftShinBone.transform.position).y) / 7f + leftAdditive;
                ...
                {
                    rightXPush = 1f + moreStupid;
                    leftXPush = -.25f;
                    rightUpPush = (transform.root.InverseTransformPoint(rightShinBone.transform.position).y) / 10f + rightAdditive;
                    leftUpPush = (transform.root.InverseTransformPoint(rightShinBone.transform.position).y) / stupid;
                    target6Push = .4f;
                    midXSlide = .35f;
                    parentVeryFrontScaleVal = .7f;
